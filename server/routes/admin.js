...
// 2.4 🗑️ ลบ User ถาวร (แต่เก็บข่าวไว้) [สำคัญ!]
router.delete('/users/:id', async (req, res) => {
    const userId = req.params.id;
    try { 
        // ขั้นตอนที่ 1: ปลดข่าวออกจาก User นี้ก่อน (Set user_id เป็น NULL) 
        // เพื่อให้ข่าวไม่หายไปพร้อมกับ User
        await db.query('UPDATE news SET user_id = NULL WHERE user_id = ?', [userId]);

        // ขั้นตอนที่ 2: ลบ User ออกจากระบบ
        await db.query('DELETE FROM users WHERE id = ?', [userId]); 
        
        res.json({ message: 'ลบผู้ใช้เรียบร้อย (ข่าวและโพสต์ยังถูกเก็บไว้)' }); 
    } catch (err) { 
        console.error(err);
...
// (อันนี้สำคัญมาก ถ้าขาดไป หน้า Home จะ Error ตอนโหลดประวัติ)
